...
import com.arlys.moviflexx.model.PushNotificationHelper;
...
public class Chat extends BaseActivity {
    ...
    /**
     * Token FCM del contacto (la otra persona en la conversación).
     * Se carga en onCreate() mediante cargarTokenContacto().
     * Una vez cargado nunca se pone a "" de nuevo.
     */
    private String fcmTokenContacto = "";

    /**
     * ID del contacto (el que no soy yo).
     * Se obtiene al procesar los mensajes o la conversación.
     * Guardarlo evita tener que buscarlo cada vez.
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        boolean nombreGenerico = nombreContacto.equals("Chat")
                || nombreContacto.equals("Conductor")
                || nombreContacto.equals("Pasajero")
                || nombreContacto.isEmpty()
                || nombreContacto.startsWith("Conductor #")
                || nombreContacto.startsWith("Pasajero #");
        if (nombreGenerico) cargarNombreDesdeConversacion();

        setEstadoConectando();

        // ── Carga el historial; al terminar intenta cargar el token ───────────
        cargarHistorial();
        arrancarPolling();
    ...
    private void mostrarFotoAmpliada() {
        ...
        if (d.getWindow() != null)
            d.getWindow().setBackgroundDrawable(new ColorDrawable(Color.parseColor("#CC000000")));
        ...
        if (iv != null) {
            if (!fotoContactoUrl.isEmpty())
                Glide.with(this).load(fotoContactoUrl)
                        .placeholder(R.drawable.logomo).error(R.drawable.logomo).into(iv);
    ...
    private void cargarFotoContactoEnHeader(ImageView ivFoto,
                                            MaterialCardView cardFoto,
                                            MaterialCardView cardInicial) {
        ConexionApi.getInstance(this).getObject(
                Constantes.CHAT_CONVERSACIONES + "/" + idConversacion,
                response -> {
                        ...
                        if (foto != null && !foto.isEmpty() && !foto.equals("null")) {
                            ...
                            if (ivFoto != null) {
                                ...
                                Glide.with(this).load(foto).circleCrop()
                                        ...
                                        .error(R.drawable.logomo).into(ivFoto);
                            }
                        }
                    });
                },
    ...
    private String resolverFotoContacto(JSONObject conv) {
        ...
        long idPas  = conv.optLong("idPasajero",  -1);
        long idCond = conv.optLong("idConductor", -1);
        JSONObject pasObj  = conv.optJSONObject("pasajero");
        JSONObject condObj = conv.optJSONObject("conductor");
        JSONObject co = (idPas == idUsuarioActual)  ? condObj
                : (idCond == idUsuarioActual) ? pasObj
                : condObj;
        if (co != null) {
            for (String c : new String[]{"fotoPerfi","fotoPerfil","foto","photoUrl","profilePicture","avatar"}) {
                String v = co.optString(c, "");
    ...
    private void cargarFotoContactoPorId(int id) {
        ConexionApi.getInstance(this).getObject(
                Constantes.authPorId((long) id),
                perfil -> {
                    String foto = "";
                    for (String c : new String[]{"fotoPerfi","fotoPerfil","foto","photoUrl","avatar"}) {
                        String v = perfil.optString(c, "");
                        if (!v.isEmpty() && !v.equals("null")) { foto = v; break; }
                    }
                    if (!foto.isEmpty()) {
                        final String ff = foto;
                        fotoContactoUrl = ff;
                        runOnUiThread(() -> {
                            ImageView        iv = findViewById(R.id.iv_mi_foto_chat);
                            MaterialCardView cf = findViewById(R.id.card_mi_foto_chat);
                            MaterialCardView ci = findViewById(R.id.card_mi_inicial_chat);
                            if (iv != null) {
                                if (cf != null) cf.setVisibility(View.VISIBLE);
                                if (ci != null) ci.setVisibility(View.GONE);
                                Glide.with(this).load(ff).circleCrop()
                                        .placeholder(R.drawable.logomo)
                                        .error(R.drawable.logomo).into(iv);
                            }
                        });
                    }
                },
                error -> Log.w(TAG, "cargarFotoContactoPorId error id=" + id));
    }
                ...
                response -> {
                    ...
                    if (nom != null && !nom.isEmpty()) {
                        ...
                        runOnUiThread(() -> {
                            ...
                            if (ta != null)
                                ta.setText(String.valueOf(nombreContacto.charAt(0)).toUpperCase());
                        });
                    }
                },
                error -> Log.w(TAG, "No se pudo cargar nombre desde conversación"));
    }

    // ══════════════════════════════════════════════════════════════════════════
    //  TOKEN FCM DEL CONTACTO
    //
    //  Estrategia en cascada:
    //    1. Buscar el ID del contacto en los mensajes del historial
    //    2. Pedir el perfil de ese usuario → extraer fcmToken
    //    3. Si falla, intentar endpoint alternativo /api/auth/usuarios/{id}
    //    4. Si el token sigue vacío al enviar, reintentar una vez más
    // ══════════════════════════════════════════════════════════════════════════

    /**
     * Punto de entrada: busca el ID del contacto en los mensajes y luego
     * carga su token FCM.  Se llama después de cargar el historial para
     * asegurarse de que listaMensajes ya tiene datos.
     */
    private void cargarTokenContacto() {
        // Si ya lo tenemos, no hacer nada
        if (!fcmTokenContacto.isEmpty()) {
            Log.d("PUSH_DEBUG", "✅ Token ya disponible: " + fcmTokenContacto.substring(0, Math.min(20, fcmTokenContacto.length())) + "...");
            return;
        }

        // Paso 1: buscar el ID del contacto en los mensajes ya cargados
        if (idContacto <= 0) {
            for (int i = listaMensajes.size() - 1; i >= 0; i--) {
                Mensaje m = listaMensajes.get(i);
                if (!m.isEsPropio() && m.getIdEmisor() > 0) {
                    idContacto = m.getIdEmisor();
                    break;
                }
            }
        }
        ...
        if (idContacto > 0) {
            Log.d("PUSH_DEBUG", "🔍 Cargando token para idContacto=" + idContacto);
            cargarTokenPorId(idContacto);
        } else {
            // Si no hay mensajes del contacto todavía, pedir los mensajes al backend
            Log.w("PUSH_DEBUG", "⚠️ No hay mensajes del contacto aún — buscando vía API");
            buscarIdContactoEnApi();
        }
    }

    /**
     * Busca el ID del contacto haciendo GET de los mensajes de la conversación.
     * Se usa cuando listaMensajes está vacío (conversación nueva).
     */
    private void buscarIdContactoEnApi() {
        ConexionApi.getInstance(this).getArray(
                Constantes.chatMensajesPorConversacion(idConversacion),
                mensajes -> {
                    int id = -1;
                    for (int i = 0; i < mensajes.length(); i++) {
                        JSONObject m = mensajes.optJSONObject(i);
                        if (m == null) continue;
                        int idEmisor = m.optInt("idEmisor",
                                m.optInt("emisorId",
                                        m.optInt("idRemitente", -1)));
                        if (idEmisor > 0 && idEmisor != idUsuarioActual) {
                            id = idEmisor;
                            break;
                        }
                    }
                    if (id > 0) {
                        idContacto = id;
                        Log.d("PUSH_DEBUG", "🔍 ID contacto encontrado vía API: " + idContacto);
                        cargarTokenPorId(idContacto);
                    } else {
                        Log.w("PUSH_DEBUG", "⚠️ Conversación sin mensajes del contacto todavía");
                    }
                },
                error -> Log.e("PUSH_DEBUG", "Error buscando ID contacto: " + error.toString())
        );
    }

    /**
     * Dado el ID del contacto, intenta obtener su token FCM.
     * Prueba primero /api/usuarios/{id} y si falla /api/auth/usuarios/{id}.
     */
    private void cargarTokenPorId(int id) {
        String url1 = Constantes.BASE_URL + "/api/usuarios/" + id;
        ConexionApi.getInstance(this).getObject(url1,
                perfil -> {
                    Log.d("PUSH_DEBUG", "📋 Perfil recibido: " + perfil.toString());
                    String token = extraerTokenDePerfil(perfil);
                    if (!token.isEmpty()) {
                        fcmTokenContacto = token;
                        Log.d("PUSH_DEBUG", "✅ Token obtenido de /api/usuarios/" + id
                                + " → " + token.substring(0, Math.min(20, token.length())) + "...");
                    } else {
                        Log.w("PUSH_DEBUG", "⚠️ /api/usuarios/" + id + " sin fcmToken — intentando ruta alternativa");
                        cargarTokenPorIdAlternativo(id);
                    }
                },
                error -> {
                    Log.w("PUSH_DEBUG", "⚠️ /api/usuarios/" + id + " falló — intentando /api/auth/usuarios/");
                    cargarTokenPorIdAlternativo(id);
                }
        );
    }

    private void cargarTokenPorIdAlternativo(int id) {
        String url2 = Constantes.BASE_URL + "/api/auth/usuarios/" + id;
        ConexionApi.getInstance(this).getObject(url2,
                perfil -> {
                    String token = extraerTokenDePerfil(perfil);
                    if (!token.isEmpty()) {
                        fcmTokenContacto = token;
                        Log.d("PUSH_DEBUG", "✅ Token obtenido de /api/auth/usuarios/" + id);
                    } else {
                        Log.e("PUSH_DEBUG", "❌ Ambos endpoints sin fcmToken para id=" + id
                                + " | Respuesta: " + perfil.toString());
                    }
                },
                error -> Log.e("PUSH_DEBUG", "❌ Ambos endpoints fallaron para id=" + id)
        );
    }

    /**
     * Extrae el valor del token FCM de un JSONObject de perfil,
     * probando múltiples nombres de campo posibles.
     */
    private String extraerTokenDePerfil(JSONObject perfil) {
        if (perfil == null) return "";
        for (String campo : new String[]{"fcmToken", "tokenFCM", "fcm_token", "token", "firebaseToken", "pushToken"}) {
    ...
    private String resolverNombreContacto(JSONObject conv) {
        ...
        JSONObject condObj = conv.optJSONObject("conductor");
        if (idPas > 0 && idCond > 0) {
            if (idPas  == idUsuarioActual) return condObj != null ? extraerNombreChat(condObj) : null;
            if (idCond == idUsuarioActual) return pasObj  != null ? extraerNombreChat(pasObj)  : null;
            return condObj != null ? extraerNombreChat(condObj) : null;
        }
        if (pasObj != null && condObj != null) {
            long idP = leerIdSeguroChat(pasObj);
            long idC = leerIdSeguroChat(condObj);
            if (idP == idUsuarioActual) return extraerNombreChat(condObj);
            if (idC == idUsuarioActual) return extraerNombreChat(pasObj);
            return extraerNombreChat(condObj);
        }
        JSONObject u1 = conv.optJSONObject("usuario1");
        JSONObject u2 = conv.optJSONObject("usuario2");
        if (u1 != null && u2 != null)
            return leerIdSeguroChat(u1) == idUsuarioActual
                    ? extraerNombreChat(u2) : extraerNombreChat(u1);
        String nom = conv.optString("nombreContacto", conv.optString("nombre_contacto", ""));
        return nom.isEmpty() || nom.equals("null") ? null : nom;
    }

    private long leerIdSeguroChat(JSONObject o) {
        if (o == null) return -1;
        for (String c : new String[]{"id","idUsuarios","idUsuario","userId","user_id"}) {
            long v = o.optLong(c, -1);
            if (v > 0) return v;
        }
        return -1;
    }

    private String extraerNombreChat(JSONObject o) {
        if (o == null) return null;
        for (String c : new String[]{"nombre","nombreCompleto","name","nombres"}) {
            String v = o.optString(c, "");
            if (!v.isEmpty() && !v.equals("null")) return v;
        }
        String n = o.optString("nombres",   "");
        String a = o.optString("apellidos", "");
    ...
    private void enviarEstadoEscribiendo(boolean escribiendo) {
        ...
        try {
            ...
            String url = Constantes.BASE_URL
                    + "/api/chat/conversaciones/" + idConversacion + "/escribiendo";
            ConexionApi.getInstance(this).put(url, body,
                    r -> {},
                    e -> Log.w(TAG, "enviarEstadoEscribiendo error"));
        } catch (Exception e) {
            Log.e(TAG, "enviarEstadoEscribiendo", e);
        }
    }
    ...
    private void verificarContactoEscribiendo() {
        ...
        ConexionApi.getInstance(this).getObject(url,
                response -> {
                    ...
                    int idEscritor = response.optInt("idUsuario",
                            response.optInt("idEmisor", -1));
                ...
                error -> {
                    if (estabaEscribiendo) {
                        estabaEscribiendo = false;
                        runOnUiThread(() -> mostrarEscribiendo(false));
                    }
                });
    }
    ...
    private void configurarScrollListener() {
        rvMensajes.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            @Override public void onScrollStateChanged(@NonNull RecyclerView rv, int s) {
                if (s == RecyclerView.SCROLL_STATE_IDLE) marcarVisiblesComoLeidos();
            }
        });
    }
            ...
            @Override public void onTextChanged(CharSequence s, int a, int b, int c) {
                ...
                if (hayTexto) {
                    ...
                    if (typingDebounceRunnable != null)
                        typingHandler.removeCallbacks(typingDebounceRunnable);
                ...
                if (hayTexto) {
                    if (btnAudio != null && btnAudio.getVisibility() == View.VISIBLE) {
                        btnAudio.animate().scaleX(0f).scaleY(0f).alpha(0f)
                                .setDuration(120)
                                .withEndAction(() -> btnAudio.setVisibility(View.GONE))
                                .start();
                        btnEnviar.setVisibility(View.VISIBLE);
                        btnEnviar.setScaleX(0f); btnEnviar.setScaleY(0f); btnEnviar.setAlpha(0f);
                        btnEnviar.animate().scaleX(1.1f).scaleY(1.1f).alpha(1f)
                                .setDuration(150)
                                .withEndAction(() -> btnEnviar.animate()
                    ...
                    if (btnEnviar.getVisibility() == View.VISIBLE) {
                        btnEnviar.animate().scaleX(0f).scaleY(0f).alpha(0f)
                                .setDuration(120)
                                .withEndAction(() -> {
                                    btnEnviar.setVisibility(View.GONE);
                                    btnEnviar.setEnabled(false);
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        if (grabando) {
                            ...
                            if (dx > dpToPx(120)) cancelarGrabacion();
                            else if (tvDeslizarCancelar != null)
    ...
    private void iniciarGrabacion() {
        ...
        try {
            rutaAudioTemp = getCacheDir().getAbsolutePath()
    ...
    private void detenerYEnviarAudio() {
        ...
        if (durMs < 1000) {
            borrarAudioTemp();
            Toast.makeText(this, "Audio muy corto", Toast.LENGTH_SHORT).show();
            return;
        }
        long seg = durMs / 1000;
        enviarMensajeAudio(rutaAudioTemp,
                seg < 60 ? seg + "s" : (seg / 60) + "m " + (seg % 60) + "s");
    }
    ...
    private void iniciarContadorGrabacion() {
        ...
        grabacionRunnable = new Runnable() {
            @Override public void run() {
                if (!grabando) return;
                long e = (System.currentTimeMillis() - inicioGrabacion) / 1000;
                if (tvTiempoGrabacion != null)
                    tvTiempoGrabacion.setText((e / 60) + ":"
                            + String.format(Locale.getDefault(), "%02d", e % 60));
    ...
    private void detenerContadorGrabacion() {
        if (grabacionRunnable != null) {
            grabacionHandler.removeCallbacks(grabacionRunnable);
            grabacionRunnable = null;
        }
    }
    ...
    private void pulsarBotonAudio(boolean activar) {
        ...
        if (activar) {
            android.animation.ObjectAnimator px =
                    android.animation.ObjectAnimator.ofFloat(btnAudio, "scaleX", 1f, 1.3f, 1f);
            px.setDuration(600);
            px.setRepeatCount(android.animation.ObjectAnimator.INFINITE);
            px.setInterpolator(new android.view.animation.AccelerateDecelerateInterpolator());
            px.start();
            android.animation.ObjectAnimator py =
                    android.animation.ObjectAnimator.ofFloat(btnAudio, "scaleY", 1f, 1.3f, 1f);
            py.setDuration(600);
            py.setRepeatCount(android.animation.ObjectAnimator.INFINITE);
            py.setInterpolator(new android.view.animation.AccelerateDecelerateInterpolator());
            py.start();
        } else {
    ...
    private void enviarMensajeAudio(String ruta, String duracion) {
        ...
        opt.setFechaEnvio(new SimpleDateFormat(
                "yyyy-MM-dd'T'HH:mm:ss", Locale.getDefault()).format(new Date()));
    ...
    private void ocultarSugerencias(boolean animado) {
        ...
        if (animado) {
            panelSugerencias.animate()
                    .translationY(panelSugerencias.getHeight()).alpha(0f)
                    .setDuration(240)
                    .withEndAction(() -> {
    ...
    private void onHistorialOk(JSONArray arr) {
        ...
        runOnUiThread(() -> {
            ...
            actualizarIndicadorPresencia();
            rvMensajes.post(this::marcarVisiblesComoLeidos);
            // ── AQUÍ se carga el token, una vez que los mensajes ya están ────
            cargarTokenContacto();
        });
    }
    ...
    private void onHistorialError(VolleyError error) {
        ...
        ConexionApi.getInstance(this).getObject(
                ...
                err2 -> runOnUiThread(() -> {
                    ...
                    // Intentar igual aunque no haya historial
                    cargarTokenContacto();
                }));
    }
    ...
    private void actualizarIndicadorPresencia() {
        if (tvEstado == null || dotEstado == null) return;
        if (estabaEscribiendo) return;
        if (!conexionActiva || fallosConsecutivos >= MAX_FALLOS_PARA_OFFLINE) {
            setEstadoSinConexion(); return;
        }
        if (timestampUltimoMensajeContacto == 0L) { setEstadoDesconocido(); return; }
        long diff = System.currentTimeMillis() - timestampUltimoMensajeContacto;
    ...
    private void setEstadoConectando() {
        ...
        tvEstado.setTextColor(Color.parseColor("#CCF5F1"));
        dotEstado.setBackgroundResource(R.drawable.circle_offline);
        dotEstado.setAlpha(0.5f);
        dotEstado.setVisibility(View.VISIBLE);
    }

    private void setEstadoSinConexion() {
        if (tvEstado == null || dotEstado == null) return;
        tvEstado.setText("Sin conexión");
        tvEstado.setTextColor(Color.parseColor("#CCF5F1"));
        dotEstado.setBackgroundResource(R.drawable.circle_offline);
        dotEstado.setAlpha(1f);
        dotEstado.setVisibility(View.VISIBLE);
    ...
    private void animarCambioEstado() {
        ...
        tvEstado.animate().alpha(0f).setDuration(140)
                .withEndAction(() -> tvEstado.animate().alpha(1f).setDuration(180).start())
                .start();
        if (dotEstado != null) {
            dotEstado.setVisibility(View.VISIBLE);
            dotEstado.animate().scaleX(1.5f).scaleY(1.5f).setDuration(140)
                    .withEndAction(() -> dotEstado.animate()
    ...
    private void procesarArray(JSONArray arr, boolean limpiar) {
        ...
        try {
            if (limpiar) {
                List<Mensaje> al = new ArrayList<>();
                for (Mensaje m : listaMensajes)
                    if (idsAudioLocal.contains(m.getId())) al.add(m);
                listaMensajes.clear();
                listaMensajes.addAll(al);
            }
            for (int i = 0; i < arr.length(); i++) {
                ...
                if (limpiar) {
                    ...
                    if (m.getId() > 0 && m.getId() <= ultimoIdVisto) {
                        for (int j = 0; j < listaMensajes.size(); j++) {
                            if (listaMensajes.get(j).getId() == m.getId()) {
                                if (listaMensajes.get(j).isEsPropio()
                                        && m.isLeido()
                                        && !listaMensajes.get(j).isLeido()) {
                                    listaMensajes.get(j).setLeido(true);
                ...
                if (!m.isEsPropio()) {
                    ...
                    if (ts > timestampUltimoMensajeContacto)
                        timestampUltimoMensajeContacto = ts;
                    // Aprovechar para guardar el idContacto mientras procesamos
                    if (idContacto <= 0 && m.getIdEmisor() > 0)
                        idContacto = m.getIdEmisor();
                }
            }
            if (limpiar) {
                Collections.sort(listaMensajes, (a, b) ->
                        Long.compare(parsearTimestamp(a.getFechaEnvio()),
                                parsearTimestamp(b.getFechaEnvio())));
    ...
    private void marcarVisiblesComoLeidos() {
        ...
        int primero = llm.findFirstCompletelyVisibleItemPosition();
        int ultimo  = llm.findLastCompletelyVisibleItemPosition();
        if (primero < 0) primero = llm.findFirstVisibleItemPosition();
        if (ultimo  < 0) ultimo  = llm.findLastVisibleItemPosition();
        if (primero < 0 || ultimo < 0) return;
        boolean cambios = false;
        for (int i = primero; i <= ultimo && i < listaMensajes.size(); i++) {
            Mensaje m = listaMensajes.get(i);
            if (!m.isEsPropio() && m.getId() > 0 && !m.isLeido()) {
                m.setLeido(true);
                idsLeidos.add(m.getId());
                cambios = true;
                enviarLeidoAlBackend(m.getId());
            }
        }
        if (cambios) rvMensajes.post(() -> adapter.notifyDataSetChanged());
    }

    private void enviarLeidoAlBackend(long id) {
        if (idsEnviadosLeido.contains(id)) return;
        idsEnviadosLeido.add(id);
        ConexionApi.getInstance(this).put(
                Constantes.chatMarcarLeido(id), null,
                r  -> Log.d(TAG, "✅ leído " + id),
                e  -> { idsEnviadosLeido.remove(id); Log.w(TAG, "⚠️ no leído " + id); });
    }
        ...
        ConexionApi.getInstance(this).getArray(url,
                arr -> {
                    ...
                    runOnUiThread(() -> {
                        ...
                        if (!estabaEscribiendo) actualizarIndicadorPresencia();
                        rvMensajes.post(this::marcarVisiblesComoLeidos);
                        // Si todavía no tenemos token, reintentar silenciosamente
                        if (fcmTokenContacto.isEmpty()) cargarTokenContacto();
                    });
                },
    ...
    private void enviarMensaje() {
        ...
        ConexionApi.getInstance(this).post(
                Constantes.CHAT_MENSAJES, body,
                response -> runOnUiThread(() -> confirmarEnvio(opt, response, texto)),
    ...
    private void guardarNotificacionEnBackend(String contenido, String miNombre) {
        if (idContacto <= 0) return;
        try {
            JSONObject body = new JSONObject();
            body.put("idUsuario",    idContacto);
            body.put("titulo",       miNombre + " te envió un mensaje");
            body.put("mensaje",      contenido);
            body.put("tipo",         "MENSAJE");
            body.put("idReferencia", idConversacion);
            body.put("leido",        false);

            ConexionApi.getInstance(this).post(
                    Constantes.BASE_URL + "/api/notificaciones",
                    body,
                    r -> Log.d("PUSH_DEBUG", "Notificación guardada en backend"),
                    e -> Log.w("PUSH_DEBUG", "Error guardando notificación: " + e)
            );
        } catch (Exception e) {
            Log.e("PUSH_DEBUG", "Error preparando notificación: " + e.getMessage());
        }
    }
    ...
    private void mostrarMenuOpciones(View anchor) {
        ...
        p.getMenu().add(0, 1, 0, "Buscar");
        p.getMenu().add(0, 2, 1, "Archivos, enlaces y docs.");
        p.getMenu().add(0, 3, 2, "Mensajes temporales");
        p.getMenu().add(0, 4, 3, "Tema del chat");
        p.getMenu().add(0, 5, 4, "Más ▶");
    ...
    private void mostrarSubMenuMas(View anchor) {
        PopupMenu s = new PopupMenu(this, anchor);
        s.getMenu().add(0, 10, 0, "Vaciar chat");
        s.getMenu().add(0, 11, 1, "Exportar chat");
        s.getMenu().add(0, 12, 2, "Crear acceso directo");
        s.getMenu().add(0, 13, 3, "Añadir a lista");
        s.setOnMenuItemClickListener(item -> {
            switch (item.getItemId()) {
    ...
    private void accionArchivosYDocs() {
        Toast.makeText(this, "Archivos, enlaces y docs.", Toast.LENGTH_SHORT).show();
    }
    private void accionTemaChat() {
        Toast.makeText(this, "Tema del chat (próximamente)", Toast.LENGTH_SHORT).show();
    }
    private void accionAccesoDirecto() {
        Toast.makeText(this, "Acceso directo a " + nombreContacto + " creado", Toast.LENGTH_SHORT).show();
    }
    private void accionAnadirALista() {
        Toast.makeText(this, "Añadido a la lista de favoritos", Toast.LENGTH_SHORT).show();
    }
    private void accionMensajesTemporales() {
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Activar 24h", (d, w) ->
                        Toast.makeText(this, "Mensajes temporales: 24h", Toast.LENGTH_SHORT).show())
    ...
    private void confirmarVaciarChat() {
        new AlertDialog.Builder(this)
                ...
                .setMessage("¿Eliminar todos los mensajes?")
    ...
    private void accionExportarChat() {
        if (listaMensajes.isEmpty()) {
            Toast.makeText(this, "No hay mensajes", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        for (Mensaje m : listaMensajes) {
            ...
            String hora  = "";
            try {
                long ts = Long.parseLong(m.getFechaEnvio());
                if (ts < 10_000_000_000L) ts *= 1000L;
                hora = " [" + sdf.format(new Date(ts)) + "]";
    ...
    private void animarBotonBack() {
        ...
        btnBack.animate().rotation(-12f).setDuration(90)
                .withEndAction(() -> btnBack.animate().rotation(0f).setDuration(90).start())
                .start();
    }
    ...
    private JSONArray extraerArray(JSONObject r) {
        ...
        if (r.has("mensajes")) return r.optJSONArray("mensajes");
        if (r.has("content"))  return r.optJSONArray("content");
        if (r.has("data"))     return r.optJSONArray("data");
        try {
            if (r.toString().startsWith("[")) return new JSONArray(r.toString());
        } catch (Exception ignored) {}
        return null;
    }
    ...
    private int buscarOptimista(String contenido) {
        for (int i = listaMensajes.size() - 1; i >= 0; i--) {
            Mensaje m = listaMensajes.get(i);
            if (m.isEnviando() && !m.isTipoAudio()
                    && contenido != null && contenido.equals(m.getContenido())) return i;
        }
        return -1;
    }
