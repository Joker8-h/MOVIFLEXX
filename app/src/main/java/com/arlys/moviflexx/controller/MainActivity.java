...
/**
 * ✈️ MOVIFLEX — SPLASH SCREEN PREMIUM
 *
 * Siempre se muestra el splash animado.
 * Al terminar decide a dónde ir según los extras:
 *   - LOGGED_IN = true  → HomeConductor o HomePasajero
 *   - LOGGED_IN = false → Home (onboarding) → Login
 */
public class MainActivity extends BaseActivity {
    ...
    private final View[] particles = new View[20];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ── Al terminar el splash decidir destino ─────────────
        handler.postDelayed(() -> {
            animateExit();
            handler.postDelayed(() -> {

                boolean loggedIn    = getIntent().getBooleanExtra("LOGGED_IN",    false);
                boolean esConductor = getIntent().getBooleanExtra("ES_CONDUCTOR", false);

                Intent intent;
                if (loggedIn) {
                    // Con sesión → ir directo al Home del rol
                    intent = new Intent(MainActivity.this,
                            esConductor ? HomeConductor.class : HomePasajero.class);
                } else {
    ...
    private void initViews() {
        ...
        for (int i = 0; i < 20; i++) {
            int resId = getResources().getIdentifier(
                    "particle_" + (i + 1), "id", getPackageName());
    ...
    private void animateRadarWave(View wave, long delay, int duration, float maxScale) {
        if (wave == null) return;
        wave.setScaleX(0f); wave.setScaleY(0f); wave.setAlpha(0f);
    ...
    private void phaseTwoEnergy() {
        if (radialGlow != null) {
            radialGlow.setScaleX(0f); radialGlow.setScaleY(0f);
    ...
    private void phaseThreeLogo() {
        ...
        if (logoCard != null) {
            logoCard.setScaleX(0.75f); logoCard.setScaleY(0.75f); logoCard.setAlpha(0f);
    ...
    private void phaseFiveLetterReveal() {
        ...
        for (int i = 0; i < letterViews.length; i++) {
            ...
            handler.postDelayed(() -> revealLetter(letter), (long) i * LETTER_DELAY);
        }
    }

    private void revealLetter(TextView letter) {
